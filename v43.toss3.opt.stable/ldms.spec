...
Requires: %{?scl_prefix}sosdb-python2, %{?scl_prefix}sosdb >= 4.2.1 
BuildRequires: %{?scl_prefix}sosdb-devel
Requires: python2
...
BuildRequires: bison bison-devel flex flex-devel, libevent >= 2.0.21
...
--with-sos=%{?scl_prefix:%_scl_root}/usr \
--disable-static \
...
%exclude %{_libdir}/ovis-ldms/libstore_sos*
...
Group: %{ldms_grp}
Requires: %{?scl_prefix}ovis = %{scl_name_version}
%description devel
...
Group: %{ldms_grp}
Requires: %{?scl_prefix}ovis = %{scl_name_version} %{?scl_prefix}ovis-initscripts-base
...
# install needs
Requires: %{?scl_prefix}ovis >= 3.0.0 python
# build needs
...
%package ldms-sos
Summary: LDMS sosdb plugins
Requires: %{?scl_prefix}sosdb
%description ldms-sos
Libraries and utilities for SOS usage in LDMS
%files ldms-sos
%defattr(-,root,root)
